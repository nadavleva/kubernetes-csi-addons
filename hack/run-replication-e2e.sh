...
if command -v stdbuf &>/dev/null; then
	USE_EXISTING_CLUSTER=true stdbuf -oL go test -v -timeout "${REPLICATION_TEST_TIMEOUT}" "${E2E_PKG}" 2>&1 | tee "${LOG_FILE}"
else
	USE_EXISTING_CLUSTER=true go test -v -timeout "${REPLICATION_TEST_TIMEOUT}" "${E2E_PKG}" 2>&1 | tee "${LOG_FILE}"
fi
