...
| `FENCE_CIDRS`                  | Comma-separated CIDRs for fencing tests. For **networkfence**, if unset: CSIAddonsNode `status.networkFenceClientStatus`, then node InternalIPs. For **iptables**, if unset: service/backend discovery (`FENCE_TARGET_SERVICES`, `FENCE_AUTO_ENDPOINT_NAMESPACES`); raw node IPs are not used as targets unless listed here. | (discovered or node IPs for NetworkFence only) |
